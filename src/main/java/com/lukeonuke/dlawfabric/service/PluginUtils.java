...
import com.lukeonuke.dlawfabric.model.backend.BanModel;
...
//    }
    public static DiscordModel authentificatePlayer(DlawFabric mod, String uuid) throws LoginException {
        try{
            ...
            if(cs.getRespectGlobalBans()){
                try {
                    BanModel ban = ds.getBanByUserDiscordId(model.getUser().getDiscordId());
                    String reason = ban.getReason();
                    if (reason == null) reason = "You have been globally banned";

                    DlawFabric.LOGGER.info("Player banned: " + reason);
                    throw new LoginException(reason);
                } catch (Exception ignored) {
                    DlawFabric.LOGGER.info("No global ban found for player");
                }
            }
            ...
            Member member;
            try {
                member = guild.retrieveMemberById(model.getUser().getDiscordId()).complete();
            } catch (ErrorResponseException er){
                throw new LoginException("Player isn't in discord server.");
            }
    ...
    public static String stripMarkdown(String s) {
        return MarkdownSanitizer.sanitize(s, MarkdownSanitizer.SanitizationStrategy.REMOVE);
    }
